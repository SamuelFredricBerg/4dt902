...
function : TYPE  ID '(' ((functionInput ',')* functionInput)? ')' '{' stmt* '}';

callFunction : ID '(' (expr ',')* (expr)? ')' ';'?;
functionInput: TYPE ID;
...
whileStatment : 'while' ('(' expr ')' '{' stmt* '}');

ifAndElseStatments : ('if') '(' expr ')' ('{' stmt* '}' | stmt) ('else'  ('{' stmt* '}' | stmt) )?;
...
TYPE : ('int' | 'int[]' | 'bool' | 'string' | 'float' | 'float[]' | 'char' | 'char[]') ;
FLOAT : ('0' | (('1'..'9') ('0'..'9'))) '.' ('0'..'9')+;
INT: '0' | ('1' ..'9') ('0' ..'9');
ID  : (('a'..'z') | ('A'..'Z'))+;
BOOLEAN : ('true' | 'false');
